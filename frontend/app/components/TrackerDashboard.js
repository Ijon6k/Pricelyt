...
export default function TrackerDashboard({ tracker }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
      {/* ── MAIN: Chart / History table ── spans 3 cols */}
              ...
              <div className="overflow-x-auto">
                {!tracker.price_logs || tracker.price_logs.length === 0 ? (
                  ...
                  <table className="w-full text-sm text-left">
                    <thead className="text-[10px] text-[rgb(var(--muted))] uppercase tracking-wider border-b border-[rgb(var(--border))]">
                      <tr>
        ...
        <div className="sticky top-24 border border-[rgb(var(--border))] rounded-lg overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgb(var(--border))] flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Newspaper size={14} className="text-[rgb(var(--muted))]" />
          ...
          <div className="overflow-y-auto max-h-[calc(100vh-10rem)] p-2 space-y-1 custom-scrollbar">
            {!tracker.news_logs || tracker.news_logs.length === 0 ? (
              ...
              tracker.news_logs.map((news) => (
                ...
                >
                  <h4 className="font-medium text-xs text-[rgb(var(--fg))] group-hover:text-[rgb(var(--accent))] leading-snug line-clamp-2 transition-colors">
                    {news.title || "Untitled"}
                  </h4>
                  <p className="text-[11px] text-[rgb(var(--muted))] line-clamp-2 mt-1">
                    {news.content}
                  </p>
                  <div className="flex items-center gap-2 text-[10px] text-[rgb(var(--muted-lighter))] mt-1.5">
